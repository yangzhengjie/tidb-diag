...
> 关联文档：[技术文档](./tidb-diag-agent-technical-design.md)<br>
> 变更说明：v1.11 文档评审修订（服务端时间窗、失败语义与缓存、分路径脱敏、RAG 来源版本、日志强观测、审计持久化）
...
| 输入解析 | 调用 Diagnostic API **之前** 从脱敏后的输入提取：时间语义（绝对窗口或 `recent_15m`）、集群、库/业务、错误码或关键词、是否变更。默认窗的绝对起止时间由 Diagnostic API 解析 |
...
| 阶段 | 必做要求 |
|------|----------|
| 1 现象澄清 | 解析线索；得到 **唯一** `cluster_id`、≤2h 绝对时间窗或 `recent_15m` 预置、关键词。用户线索仅作锚点。无唯一集群则不得进入阶段 2 |
...
| 规则 | 要求 |
|------|------|
| 集群 | 工具调用必须带配置中存在的 `cluster_id`。禁止猜测。P0 建立 **库名/业务名 → 集群** 映射；Diagnostic API 配置是唯一事实源，发布流水线从同一配置生成 Dify 只读变量，禁止人工维护第二份。每次诊断记录 `config_digest` |
| 映射不唯一 | 同一库名/业务名对应 **多个** 集群，或未命中映射 → 视为无唯一集群，追问（至多 2 次），不得挑一个 |
| 默认集群 | 仅当客户在 P0 **书面指定** 单集群默认值时可用；多集群环境无默认 |
| 默认时间窗 | **有集群、用户未给时间**：用默认窗，**不**因此判信息不足。只说「刚刚 / 刚才 / 最近」或完全没给时间：首个取数请求传 `time_preset=recent_15m`，由 Diagnostic API 按服务端时钟解析为 **向前 15 分钟、向后 0**；响应返回 `effective_start_time` / `effective_end_time`，本回合后续工具复用该绝对窗口 |
| 有锚点无时长 | 用户只给一个时刻：下界为该时刻前 15 分钟，上界为 `min(该时刻后 15 分钟, API now)`；锚点本身超过 now+60s 时先请用户纠正，不查询未来 |
| 仅给开始时刻 | 「从 14:30 起 / 14:30 开始」表示 `[14:30, now]`，不是孤立锚点；若区间 >2h，须请用户确认更小子窗。无法判断是孤立时刻还是开始时刻时先追问，不自行套用 ±15 分钟 |
| 日期/时段话术 | 「今天 / 上午 / 下午」先映射到日历区间：上午 09:00–12:00，下午 12:00–18:00，今天 09:00–当前。当前在区间内则上界为 now；当前早于区间起点时不得查询未来，须请用户确认日期/窗口；「今天」在 09:00 前同样须确认。**不**套 15 分钟默认窗 |
...
| 时区与合法性 | 默认 **Asia/Shanghai（+08:00）**。无时区偏移的时间按此时区解释。必须满足 `start_time < end_time`；结束时间最多允许比 API 时钟快 60 秒（时钟偏差），更晚则返回 400 `future_window`。默认窗与活跃故障判断以 **Diagnostic API 服务器时钟** 为准；报告中写明时区 |
| 用户明确拒绝时间 | 不愿使用默认窗且不提供可执行窗口 → 信息不足报告 |

默认窗由服务端解析，且按活跃故障规则 **自动 cache bust**（见 §3.8）。5 分钟缓存主要用于 **历史窗口** 的复查与追问，而不是「刚刚」类默认窗。
...
| G13 | 默认时间窗（行为） | 有唯一集群、只说「刚刚连接超时」，不给起止时间 | 不追问时间；首个取数请求使用 `time_preset=recent_15m`；API 返回的实际窗口为服务端 now−15min～now；后续调用复用该窗口；第 9 节标明默认窗；`cache_hit=false` | 追问时间；由模型时钟自行拼窗；或后续工具使用不一致窗口 |
...
**夹具包（P2 开工门禁）**：实施方可起草 G1–G8/G6a/G13/G14，但须由客户 DBA 或独立验收 Owner 在调优前审核、冻结并记录 hash。每个用例含输入原文、`cluster_id`（G5 除外）、时间窗或「用默认窗」、期望根因短句（G5/G13/G14 可为行为断言）、必见工具列表、允许降级条件、禁止结论，以及相关 Prom 模板、日志签名、慢查字段的期望方向与量化阈值。另由验收 Owner 为 G1–G8、G6a、G14 各保留至少 1 个不向实施方公开的盲测变体；G7 的失败注入方式见技术文档。公开夹具与盲测集未冻结不得开始 P2 联调。
...
| 工具轨迹 | 每次诊断可导出：工具名、参数摘要（已脱敏）、各子查询状态、`source_status` / `failure_scope`、API 实际时间窗、数据水位、`config_digest`、响应 hash；Dify 测评包另含 Prompt/模型/`kb_snapshot_id`。P2 起作为金标准验收输入 |
...
**输入**：「14:30–14:45 应用报 TiDB 连接超时」（集群已明或可映射）

1. 锚定窗口 14:30–14:45（若真实输入是「14:30 起」，按 §3.7 解释为从 14:30 到 now，超过 2h 时先缩小）
...
1. 不追问时间；选择语义窗口 `recent_15m`（实际起止时间由 API 服务端时钟解析）
2. 首个取数请求传 `time_preset=recent_15m`，后续工具复用 API 返回的实际窗口；请求会自动 cache bust
3. 第 9 节写明「默认窗」
...
| P2 | **公开夹具与盲测集已冻结**；G1、G2、G5、**G13/G14** 行为可跑通；模型前输入脱敏抽测通过；Plan B Workflow + 最小 RAG **已交付且默认关闭**；抽测「有集群但拒绝时间」→ 信息不足且无取数；09:00 前说“今天”及当前早于“上午/下午”起点时会追问而不查未来；工具轨迹可导出（报告结构可先不打根因分） |
...
| Prom label 与映射不一致 | 查错集群 | 唯一配置制品生成 Dify alias；发布校验 `config_digest`；API 校验 `cluster_id` |
