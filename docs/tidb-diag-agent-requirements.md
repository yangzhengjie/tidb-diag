...
> 变更说明：v1.17 补充开场路由与 Prompt/RAG 分工；与技术设计同步
...
**P2 必须交付** 该 Workflow（默认关闭）及覆盖 G1/G2/G3 对应附录 D 问题点的最小官方 RAG 包，确保触发时 Workflow 已能走通 Must 主路径；P3 再补齐附录 D 全量 Must 问题点与检索验收。触发后只切换入口，不在触发当周新开发。
...
| P2 慢查 + Dify 联调 | 1 周 | 慢查 raw 解析 + Agent + **4 个 Must 工具** + 含开场路由的 Prompt + Plan B Workflow（默认关闭）+ G1/G2/G3 对应问题点最小 RAG；**公开夹具与盲测集已冻结**；G1/G2/**G3**/G5/G13 行为可跑通 | 无 |
