...
> 变更说明：v1.13 将内网 Diagnostic API 认证简化为单一共享 Key，暂不实现授权与审计能力
...
| **模型输入保护** | 用户文本、附件/OCR、工具响应与 RAG 片段分别在各自可信边界执行确定性脱敏，并在模型网关作最终扫描，任一路径失败均不得调用主推理模型 |
...
| **Won't（v1）** | 告警联动、Dashboard 代理、飞书/钉钉推送、多版本、自动修复、RBAC/集群授权、审计能力、任意 PromQL | — |
...
| 安全 | 内网服务使用单一共享 Key 完成基本认证；敏感数据须在进入模型前脱敏 |
...
L3 若启用，须在离线导入和 Embedding 之前完成与[技术架构与设计文档 §8.2](./tidb-diag-agent-technical-design.md#82-数据脱敏)同等级的确定性脱敏；未经脱敏的内部案例不得进入知识库。
...
| P2 | **公开夹具与盲测集已冻结**；G1、G2、G5、**G13/G14** 行为可跑通；模型前输入脱敏抽测通过；Plan B Workflow + 最小 RAG **已交付且默认关闭**；集群下拉无默认值且包含全部已配置集群；参数与文本冲突会确认；`custom` 缺时间或窗口 >2h 时不取数；Dify 工具调用记录可导出（报告结构可先不打根因分） |
...
| 12 | 工具调用记录 | Dify 测评导出可包含工具名、脱敏参数、子查询/源状态、配置与模型/RAG 版本、响应 hash |
| 13 | 历史回放 | G9–G12 有则须「对」；无则缺口签字，**不挡上线** |
| 14 | Dify 参数 | 集群下拉无默认值且展示全部已配置集群；`recent_15m`/`custom` 映射正确；缺失、冲突、未知集群和 >2h 均不会发起无效取数 |
