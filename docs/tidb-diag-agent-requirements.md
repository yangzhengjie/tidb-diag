...
> 变更说明：v1.12 增加 Dify 受控输入参数，并收敛为需求与技术架构两份开发前技术基线
...
| **安全可审计** | v1 为 **应用级身份**（Dify 共用诊断 Key）+ **必传** `conversation_id`；诊断回合优先带 `round_id`；用户文本、附件/OCR、工具响应与 RAG 片段分别在各自可信边界执行确定性脱敏，并在模型网关作最终扫描，任一路径失败均不得调用主推理模型 |
...
| **错误日志（图片）** | Should | 终端/告警/IM 截图 | 仅在合规 OCR 输出能先脱敏再进入主模型时辅助定方向；否则 **改要脱敏后的文字**，不阻塞主路径 |
...
| 输入预处理 | 用户文本须先经确定性脱敏；图片仅在具备合规 OCR + 脱敏前置链路时启用，否则改要脱敏后的文字。工具响应由 Diagnostic API 脱敏，RAG 内容在入库前脱敏；四类内容在模型网关最终扫描通过后才可进入主推理模型 |
| 输入解析 | 调用 Diagnostic API **之前**先校验结构化参数，再从脱敏后的对话提取错误码、关键词、变更和其他线索。参数与对话中的集群/时间冲突时必须请用户确认，未确认不得取数 |
...
- **必做**（有数据则必须做，失败则按 §3.8 降级）：阶段 1 澄清；在集群与时间窗明确后按 §3.3.1 的唯一矩阵取 health、相关指标及日志/慢查；至少一次与本次假设相关的 RAG；融合并出报告。必调工具只有在返回 `error` / `partial` 且轨迹记录降级原因时才允许缺少有效结果，不能以模型自行判断“无须调用”代替。
...
L3 若启用，须在离线导入和 Embedding 之前完成与 §4.3 同等级的确定性脱敏；未经脱敏的内部案例不得进入知识库。
...
| 选项来源 | Diagnostic API 集群配置是唯一事实源。发布流水线从该配置为 Dify 生成选项，且只包含当前 `diagnostic_reader.allowed_clusters`；禁止在 Dify 人工维护第二份列表。参数不是授权凭据，API 仍须二次校验 Key 的集群权限 |
...
| 参数、鉴权、越权或回合上限错误 | 不计为观测源失败，不参与置信度；可修正的参数错误至多修正一次，鉴权/越权立即停止取数，策略错误按已有证据出降级报告 | 视是否仍能完成必做集合 |
| 知识库检索失败 | 仍出报告；建议标「待与官方文档核对」；不得标高 | 否 |
| 超出范围（§3.3.2） | 声明超出 + 转人工 | 否（正确拒答） |

`source_status` 仅描述通过鉴权和参数校验后的**数据获取结果**：`ok`（无子查询错误且至少一项有数据）/ `partial`（部分子查询或解析单元失败）/ `empty`（成功但 0 命中）/ `error`（上游超时或上游 5xx，未取得可用数据）。health 必须返回每个模板的结构化状态；有成功/空结果也有失败时为 `partial`，全部失败为 `error`，全部成功但均无序列为 `empty`。HTTP 4xx 错误另带 `failure_scope=auth|request|policy|source`；只有 `source` 可按观测源不可用降级，鉴权失败不得伪装成数据源错误。
...
| G14 | 模型前脱敏（行为） | 用户文本分别含 Token、手机号、邮箱和带常量 SQL；图片链路启用时另含截图样本 | 主模型调用记录与工具参数中均无明文；诊断仍保留错误码、SQL 骨架和安全派生特征 | 任一敏感原文进入主模型，或脱敏后无法继续基本诊断 |
| G9–G12 | 客户历史单 | P0 提供 3–4 个已结案故障（可打码） | 根因命中为「对」 | 仅文风像报告 |

**夹具包（P2 开工门禁）**：实施方可起草 G1–G8/G6a/G13/G14，但须由客户 DBA 或独立验收 Owner 在调优前审核、冻结并记录 hash。每个用例含输入原文、`cluster_id`（G5 缺参子用例除外）、`time_mode` 与对应时间值、期望根因短句（G5/G13/G14 可为行为断言）、必见工具列表、允许降级条件、禁止结论，以及相关 Prom 模板、日志签名、慢查字段的期望方向与量化阈值。另由验收 Owner 为 G1–G8、G6a、G14 各保留至少 1 个不向实施方公开的盲测变体；G7 的失败注入方式见技术架构与设计文档。公开夹具与盲测集未冻结不得开始 P2 联调。

**打分**：诊断类用例按结构分 + 根因命中（**对 / 部分对 / 错**）+ 幻觉扣分；行为类用例按行为断言判定。**「部分对」不算通过。** 所有公开夹具，以及为 G1–G8、G6a、G14 配置的全部盲测变体必须通过，才算 P3 通过。G13/G14 在 P2 先验收、P3 复测。G9–G12：**有数据则必须测且为「对」**；无数据则书面缺口，**不挡 P3、不挡 P5 上线**，验收报告须附「无真实历史回放」说明。
...
固定顺序（与技术架构与设计文档一致，不得再写成省略 metrics 的版本）：

`解析输入 → get_cluster_health → query_prometheus → fetch_component_logs → analyze_slow_query → RAG → 报告`

仍须遵守：参数缺失或冲突未确认时不进入取数；窗口 >2h 先请用户缩小；Header 必带 `X-Conversation-Id`。
...
**压测基线（P0 冻结实际数值，验收不得只写“夹具规模”）**：单集群并发 5 个完整对话，持续 ≥15 分钟且每类接口成功样本 ≥100；同时覆盖冷缓存与热缓存并分别出数。最大场景使用 2h 查询窗，health 另读取紧前 2h 对比窗；Prom `step=1m`，运行日志命中规模、慢日志 raw 扫描行数（上限 2000）及 SLS/Prom 数据基数须记录。成功样本定义为 HTTP 2xx 且 `source_status` 为 `ok` 或 `empty`；`partial`、`error` 和系统产生的 HTTP 非 2xx 均计入错误率，预先标记的非法请求安全用例单列、不混入稳定性分母。P95 只对成功样本统计，超时/错误率须 <1%；端到端 P95 按真实 Workflow 顺序直接测量，不能由各接口 P95 相加推导。

### 4.3 安全与审计

| 项 | 要求 |
|----|------|
| 认证 | HTTPS + API Key（**应用级**）：Dify 使用独立 `diagnostic_reader` Key；审计查询使用另一把 `auditor` Key。Key 须支持标识、到期、轮换与吊销 |
| 会话 | 每个 API 请求 **必须** 带 `X-Conversation-Id`（或等价字段）；缺省返回 400 |
| 回合 | 鼓励带 `X-Diag-Round-Id`；缺省走 15 分钟兜底（见 §4.2） |
| 授权 | v1 收敛为 `diagnostic_reader`（仅 4 个取数接口）与 `auditor`（仅脱敏审计/轨迹）两类 Key；互不继承权限。v1 不提供管理 REST，集群/Key 变更走配置发布 |
| 集群隔离 | 两类 Key 均绑定 `allowed_clusters`；`auditor` 只能查询其授权集群的脱敏轨迹 |
| 审计 | 全量请求写入集中式、持久化、访问受控的审计存储，默认保留 180 天；不得仅依赖 API 实例本地盘。用 `request_id` + `conversation_id`（+ `round_id`）关联 Dify 对话。记录 `config_digest`、响应 hash、Prompt/模型/`kb_snapshot_id`；v1 **不承诺** 追溯到具体员工 |
| 脱敏 | 进入主推理模型前处理 **用户文本、附件/OCR、全部工具响应和 RAG 片段**：密码、Token、DSN、手机号、身份证、**邮箱、账号字面量、SQL 字面量（字符串/数字常量）**。保留 SQL 骨架与 digest。各路径先在来源边界处理，模型网关对最终 payload 作确定性扫描；发现漏网内容时失败关闭并记录安全事件 |
| SQL 进模型 | 允许进入模型的是 **脱敏后的 SQL 骨架**及安全派生特征（如字面量数量、IN 列表规模分桶、LIMIT 分桶）；原始业务常量不进入 LLM。默认不保留原始 SQL；客户书面要求留存时须独立加密、单独授权、设置期限并审计每次读取 |
| 凭证 | SLS AK/SK 只在 Vault，**不进入 Dify** |
| 工具轨迹 | 每次诊断可导出：Dify 结构化输入参数、工具名、参数摘要（已脱敏）、各子查询状态、`source_status` / `failure_scope`、API 实际时间窗、数据水位、`config_digest`、响应 hash；Dify 测评包另含 Prompt/模型/`kb_snapshot_id`。P2 起作为金标准验收输入 |
...
| P2 慢查 + Dify 联调 | 1 周 | 慢查 raw 解析 + Agent + **4 个 Must 工具** + 模型前输入脱敏 + Plan B Workflow（默认关闭）+ G1/G2/G3 最小 RAG；**公开夹具与盲测集已冻结** | 无 |
| P3 知识库 + 金标准 | 1 周 | 完整 RAG 导入 + Prompt + G1–G8、G6a 公开及盲测；G13/G14 复测 | 无 |
...
- [ ] Prometheus URL、**集群 label 实际键名**、`cluster_id` / `display_name` 列表；Dify 下拉选项与 `diagnostic_reader.allowed_clusters` 一致
- [ ] Prom 模板强观测阈值（方向、绝对值/比例、零基线规则）、运行日志签名及最少次数/主机数/单条即强规则、慢查阈值，与压测数据规模/基数
- [x] TiDB **v7.5.6**
- [ ] TiUP 版本（默认按 v1.14 文档）
- [ ] Dify 地址、千问接入；支持必选集群下拉框、`time_mode` 与自定义起止时间校验；是否具备识图（不影响 Must）
- [ ] 安全要求（集中审计存储、留存期、写入持续失败时拒绝请求或显式降级的策略、网络隔离）
...
- [ ] 用户文本的模型前确定性脱敏节点可用；图片仅在合规 OCR + 脱敏前置链路通过抽测后开启
- [ ] 内网模型网关能检查 Agent 的最终模型 payload 并在命中敏感原文时失败关闭；若不能覆盖所有模型调用，原生 Agent 入口不得上线，须改走具备逐路径脱敏节点的 Workflow/应用层编排

### 6.2 各阶段验收要点

| 阶段 | 验收标准 |
|------|----------|
| P1 | SLS 能返回 tidb ERROR 摘要；Prom 能返回 QPS/P99 摘要；`time_preset=recent_15m` 由服务端解析并返回实际窗口；无 `X-Conversation-Id` 返回 400；未授权 401；审计可查；反向窗口、未来 >60s、窗口 >2h 均返回对应 400 且不截断；活跃窗自动 bust；失败/部分失败不入缓存 |
| P2 | **公开夹具与盲测集已冻结**；G1、G2、G5、**G13/G14** 行为可跑通；模型前输入脱敏抽测通过；Plan B Workflow + 最小 RAG **已交付且默认关闭**；集群下拉无默认值且只含授权集群；参数与文本冲突会确认；`custom` 缺时间或窗口 >2h 时不取数；工具轨迹可导出（报告结构可先不打根因分） |
| P3 | 附录 B 错误码精确 Top-3 命中；五类 Troubleshooting 可检索；全部公开夹具及 G1–G8/G6a/G14 盲测均通过；G13/G14 行为复测通过 |
...
| 6 | 诊断质量 | 全部公开夹具及 G1–G8/G6a/G14 盲测均通过（诊断类「部分对」不算）；错误根因不得标「高」；G13/G14 行为通过 |
| 7 | RAG | 附录 B 错误码精确命中 + 五类 Troubleshooting 可检索；离线 MD；来源真实版本不被改写；报告含 `kb_snapshot_id` 与可复现引用 |
| 8 | 安全 | Key 鉴权；越权拒绝；缺会话 ID 拒绝；审计可关联到会话 |
| 9 | 脱敏 | 用户文本、附件/OCR、工具响应、RAG 进入主模型前均无明文密码/Token/证件号/邮箱/SQL 常量 |
| 10 | 降级 | G5/G7 行为符合 §3.8 |
| 11 | 稳定性 | 按 §4.2 冻结基线压测：并发 5 对话；health/metrics/logs P95 < 5s；慢查 raw P95 < 20s；错误率 <1%；30s 为超时；**每个诊断回合** Diagnostic API 调用 ≤12（RAG 不计） |
| 12 | 工具轨迹 | 可导出工具名、脱敏参数、子查询/源状态、配置与模型/RAG 版本、响应 hash |
| 13 | 历史回放 | G9–G12 有则须「对」；无则缺口签字，**不挡上线** |
| 14 | Dify 参数 | 集群下拉无默认值且仅展示授权集群；`recent_15m`/`custom` 映射正确；缺失、冲突、篡改和 >2h 均不会发起越界取数 |
...
| Dify 集群选项陈旧或被篡改 | 查错/越权集群 | 唯一配置生成下拉选项并校验 `config_digest`；API 按 Key 二次校验 `cluster_id` |
